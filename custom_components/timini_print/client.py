...
def _request_raw_json(host: str, port: int, method: str, path: str, timeout: float) -> dict:
    """Like _request(), but returns the raw parsed JSON dict as-is,
    for endpoints (like /list-models) that don't use the standard
...
def list_models(host: str, port: int, timeout: float = 15.0) -> list[dict]:
    """Fetch and return TiMini-Print's own known printer model keys via
    the add-on's /list-models endpoint (which itself runs
    `timiniprint_command_line.py --list-models`). Each item is
    ``{"key": <model key>, "label": <full descriptive line>}``.
    """
    data = _request_raw_json(host, port, "GET", "/list-models", timeout)
...
def print_text(
    ...
    if printer_model:
        body["printer_model"] = printer_model
